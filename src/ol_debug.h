...
#define ol_logf(level, ...)          \
    do { ol_log_printf(level,        \
                       __FILE__,     \
                       __LINE__,     \
                       __FUNCTION__, \
                       __VA_ARGS__); } while (0)
...
#define ol_debugf(...) \
    do { ol_logf(OL_DEBUG, __VA_ARGS__); } while (0)
#define ol_debug(...) \
    do { ol_logf(OL_DEBUG, "%s\n", __VA_ARGS__); } while (0)
#define ol_errorf(...) \
    do { ol_logf(OL_ERROR, __VA_ARGS__); } while (0)
#define ol_error(...) \
    do { ol_logf(OL_ERROR, "%s\n", __VA_ARGS__); } while (0)
#define ol_infof(...) \
    do { ol_logf(OL_INFO, __VA_ARGS__); } while (0)
#define ol_info(...) \
    do { ol_logf(OL_INFO, "%s\n", __VA_ARGS__); } while (0)
#define ol_assert(assertion)                                     \
    do {                                                         \
        if (!(assertion)) {                                      \
...
#define ol_assert_ret(assertion, ret)                            \
    do {                                                         \
        if (!(assertion)) {                                      \
            ol_logf(OL_ERROR, "assert %s failed\n", #assertion); \
            return (ret);                                        \
